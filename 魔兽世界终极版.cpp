...
public:
    ...
    dragon(bool _colour, int _monster_id):monster(_colour, hp[0], _monster_id, strength[0]){
        ...
        morale = (commands[_colour].command_hp) / hp[0];
        // if(_colour)
...
void change_flag(city * citys){
    for(int i = 1; i <= N; i++){
        if(citys[i].blue_flag == 2 && citys[i].flag){
            time.get_time();
            citys[i].blue_flag = 0;
            printf("blue flag raised in city %d\n", i);
        }
        if(citys[i].red_flag == 2 && !citys[i].flag){
            time.get_time();
            citys[i].red_flag = 0;
